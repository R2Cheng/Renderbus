...
def read_excel(sheet,row,col):
    '''读取excel表公共方法'''
    a = xlrd.open_workbook('C:/adminApi/datas/apiTestCase2.xls')   #打开excel文件，apiTestCase3.xls是正式环境文件，apiTestCase2.xls是测试环境文件
...
def write_excel(list,row,col,data):
    '''写入excel表公共方法'''
    file = r"C:/adminApi/datas/apiTestCase2.xls"              #定义变量file
...
def setlogging():
    ...
    handler = logging.FileHandler('C:/adminApi/logs/' + t + ".log") # log_path = os.path.dirname(os.getcwd()) + '/Logs/'  # 项目根目录下/Logs 保存日志
