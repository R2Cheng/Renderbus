...
class Template_mixin(object):
    ...
    DEFAULT_DESCRIPTION = ''
    DEFAULT_TESTER='测试组自动化测试'

    # ------------------------------------------------------------------------
    # HTML Template

    HTML_TMPL = r"""<?xml version="1.0" encoding="UTF-8"?>
<html xmlns="zuowangwang.wang">
<head>
    <title>%(title)s</title>
...
<a href='javascript:showCase(0)' class="btn btn-sm btn-primary">概要:  %(passrate)s </a>
<a href='javascript:showCase(1)' class="btn btn-sm btn-danger">失败:  %(fail)s </a>
<a href='javascript:showCase(2)' class="btn btn-sm btn-success">通过:  %(Pass)s </a>
<a href='javascript:showCase(3)' class="btn btn-sm btn-info">所有用例数:  %(count)s </a>
</p>
...
<tr id='%(tid)s' class='%(Class)s'>
    ...
    <div id='div_%(tid)s' class="collapsed " style="display:none">
...
class HTMLTestRunner(Template_mixin):
    ...
    def getReportAttributes(self, result):
        ...
        if status:
            status = '，'.join(status)
            # if str("%.2f%%" % (float(result.success_count) == 0:
            #     self.passrate = 0
            # else:
